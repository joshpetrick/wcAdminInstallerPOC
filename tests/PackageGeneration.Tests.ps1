Describe 'Package generation path handling' {
  ...
  It 'stage wrapper streams failures to console and prepare-linux has AlmaLinux prerequisite fallback' {
    ...
    $prepare | Should -Match 'glibc-devel'
    $prepare | Should -Match 'glibc-static'
    $prepare | Should -Match 'ensure_build_tools'
    $prepare | Should -Match '/usr/bin/make'
    $prepare | Should -Match 'ensure_oracle_link_inputs'
    $prepare | Should -Match '/usr/lib64/crti.o'
    $prepare | Should -Match '/usr/lib64/libc_nonshared.a'
    $prepare | Should -Match '/usr/lib64/libpthread_nonshared.a'
