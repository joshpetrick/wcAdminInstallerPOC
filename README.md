...
Stage `06-create-database` can legitimately spend a while at `Creating Pluggable Databases`, but it should not be silent for hours. The stage now runs DBCA with `profile.oracle.databaseCreationTimeoutMinutes` (default `90`) and prints heartbeat lines every `profile.oracle.databaseCreationHeartbeatSeconds` (default `120`) while DBCA is still running. If DBCA times out or exits with a hard failure, the stage prints recent DBCA logs before failing so the next retry has actionable details instead of appearing hung. Oracle DBCA can also return exit code `6` after reporting `Database creation complete` when post-creation recompilation emits warnings such as invalid optional multimedia/spatial objects; for this POC the stage treats that as success-with-warnings only after it verifies that the CDB opens in `READ WRITE` mode and saves PDB state.
